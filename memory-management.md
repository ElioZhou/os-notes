...
**Protection bits**: 'kernel' + 'w' specifies who and what can be done on this page if kernel bit is set then only the kernel can translate this page. If user accesses the page a 'privilege exception' will be raised. If _writeprotect_ bit is set the page can only be read (load instruction). If attempted write (store instruction), a write protection exception is raised.
